...
function iniciarPacman(nivel = 1) {
    if (game && game.scene && game.scene.keys && game.scene.keys['Nivel'+nivel]) {
        ...
        const anchoBase = 480, altoBase = 600;
const config = {
    type: Phaser.AUTO,
    width: anchoBase,
    height: altoBase,
...
class PacmanNivel extends Phaser.Scene {
    ...
    preload() {
        ...
        // Cargar fotos de "familia" para la carcel y liberación
        const fotosFamilia = ['familia.jpg', 'familia1.jpg', 'familia3.jpg', 'familia4.jpg'];
        fotosFamilia.forEach(foto => this.load.image(foto, foto));
        for (let i = 1; i <= 2; i++) {
    ...
    create(data) {
    // --- SOPORTE UNIVERSAL TOUCH (SWIPE y HOLD) ---
    ...
    // Define handlers y los guarda en la instancia
    this.pointerDownHandler = (pointer) => {
        this.touchStartX = pointer.x;
        this.touchStartY = pointer.y;
        this.holdDirection = null;
        this.holdInterval = setInterval(() => {
            if (this.holdDirection) this.direccionTactil = this.holdDirection;
        }, 30);
    };
    this.pointerMoveHandler = (pointer) => {
        ...
        if (Math.abs(dx) > Math.abs(dy)) {
            if (dx > 2) this.holdDirection = 'right';
            else if (dx < -2) this.holdDirection = 'left';
        } else {
            if (dy > 2) this.holdDirection = 'down';
            else if (dy < -2) this.holdDirection = 'up';
        }
        // Movimiento inmediato al detectar swipe
        if (this.holdDirection) this.direccionTactil = this.holdDirection;
    };
    ...
    // Limpia listeners al destruir la escena
    this.events.once('shutdown', () => {
        ...
        // --- CÁRCEL CON FOTO FAMILIAR ---
        // Determinar la foto de la "carcel" según el nivel
        const fotosFamilia = ['familia.jpg', 'familia1.jpg', 'familia3.jpg', 'familia4.jpg'];
        let idxFoto = (this.nivel-1) < fotosFamilia.length ? (this.nivel-1) : fotosFamilia.length-1;
        let fotoCarcel = fotosFamilia[idxFoto];
        // Dibuja la "carcel" (rectángulo) y la foto pequeña en el centro
        this.carcelGroup = this.add.group();
        let carcelX = 240, carcelY = 300;
        let carcelGfx = this.add.graphics();
        let carcelSize = 70;
        // Dibuja fondo blanco
        carcelGfx.fillStyle(0xffffff, 0.97);
        carcelGfx.fillRect(carcelX-carcelSize/2+2, carcelY-carcelSize/2+2, carcelSize-4, carcelSize-4);
        // Dibuja marco
        carcelGfx.lineStyle(5, 0x444444, 1);
        carcelGfx.strokeRect(carcelX-carcelSize/2, carcelY-carcelSize/2, carcelSize, carcelSize);
        // Barrotes (guardamos para animar)
        let barroteColor1 = 0xdddd44, barroteColor2 = 0x888888;
        let barroteState = true;
        let dibujarBarrotes = () => {
            carcelGfx.clear();
            carcelGfx.fillStyle(0xffffff, 0.97);
            carcelGfx.fillRect(carcelX-carcelSize/2+2, carcelY-carcelSize/2+2, carcelSize-4, carcelSize-4);
            ...
            carcelGfx.strokeRect(carcelX-carcelSize/2, carcelY-carcelSize/2, carcelSize, carcelSize);
            // Barrotes verticales
            let color = barroteState ? barroteColor1 : barroteColor2;
            carcelGfx.lineStyle(4, color, 1);
            for(let i=1;i<=2;i++) carcelGfx.lineBetween(carcelX-carcelSize/2+carcelSize*i/3, carcelY-carcelSize/2, carcelX-carcelSize/2+carcelSize*i/3, carcelY+carcelSize/2);
            // Barrotes horizontales
            for(let i=1;i<=2;i++) carcelGfx.lineBetween(carcelX-carcelSize/2, carcelY-carcelSize/2+carcelSize*i/3, carcelX+carcelSize/2, carcelY-carcelSize/2+carcelSize*i/3);
        };
        dibujarBarrotes();
        let animBarrotes = this.time.addEvent({ delay: 400, loop: true, callback: ()=>{ barroteState=!barroteState; dibujarBarrotes(); } });
        // Signo de interrogación grande
        let signo = this.add.text(carcelX, carcelY, '?', { font: '42px Arial', color: '#f7b600', fontStyle: 'bold', stroke:'#444', strokeThickness:4 }).setOrigin(0.5).setDepth(2);
        this.carcelGroup.addMultiple([carcelGfx, signo]); // Solo objetos visuales, no el TimerEvent
        ...
        this.physics.add.overlap(this.jugador, this.animalesGroup, (jugador, collider) => {
            ...
            if (this.animalesRestantes <= 0) {
                // Liberar la foto: mostrar grande y esperar toque
                this.carcelGroup.clear(true, true);
                let fotoGrande = this.add.image(240, 300, fotoCarcel).setDisplaySize(220, 220).setDepth(10);
                let marco = this.add.graphics();
                marco.lineStyle(10, 0xffd700, 1);
                marco.strokeRect(240-112, 300-112, 224, 224);
                marco.setDepth(11);
                let texto = this.add.text(240, 520, '¡Gracias por liberarme, Josefita!', { font: '28px Arial', fill: '#e84118', backgroundColor:'#fff', padding: {x:12,y:6} }).setOrigin(0.5).setDepth(12);
                // Esperar cualquier toque para pasar de nivel
